...
<html lang="en">
    <head>
        ...
        <title><?php echo filter_var($label_language_values['reset_password']);	?> | <?php  echo filter_var($setting->get_option("ld_page_title")); ?></title>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo filter_var(SITE_URL, FILTER_VALIDATE_URL); ?>assets/css/font-awesome/css/font-awesome.css" /> <script type="text/javascript" src="<?php echo filter_var(SITE_URL, FILTER_VALIDATE_URL); ?>assets/js/jquery-2.1.4.min.js"></script> 
		...
		<script type="text/javascript" src="<?php echo filter_var(SITE_URL, FILTER_VALIDATE_URL); ?>assets/js/client_forgot_password.js"></script>  
								...
								<form id="reset_new_passwd" method="post">
									...
									<h1 class="forget-password"><?php echo filter_var($label_language_values['reset_password']);	?></h1>
										<div class="form-group fl">
											<label for="n_password"><i class="icon-lock"></i><?php echo filter_var($label_language_values['new_password']);	?></label>
											<input type="password" id="n_password" name="n_password" onkeydown="if (event.keyCode == 13) document.getElementById('reset_new_password').click()" placeholder="<?php echo filter_var($label_language_values['new_password']);	?>" class="showpassword npassword">
										</div>
										<div class="form-group fl">
											<label for="rn_password"><i class="icon-lock"></i><?php echo filter_var($label_language_values['retype_new_password']);	?></label>
											<input type="password" id="rn_password" name="rn_password" placeholder="<?php echo filter_var($label_language_values['retype_new_password']);	?>" class="showpassword rnpassword" onkeydown="if (event.keyCode == 13) document.getElementById('reset_new_password').click()" />
										</div>
									...
									<?php  } 	if($current_time > $url_time){	?>
										<h4 class="txt-danger" style="display:block;"><?php echo filter_var($label_language_values['your_reset_password_link_expired']);	?></h4>									
									<?php 
										}else{
									?>
									<div class="clearfix"> 
										<a class="btn ld-reset-btn btn-lg ld-xs-12 col-xs-12" href="javascript:void(0)" id="reset_new_password" type="submit"><?php echo filter_var($label_language_values['reset_password']);	?></a>
									</div>
									<?php 
										}
									?>								
