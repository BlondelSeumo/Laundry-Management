...
class laundry_provider_off_day{
	...
	function create_monthoff() {
        ...
        for($i=1;$i<=$month_size;$i++) {
            ...
            $result ="INSERT INTO `ld_off_days` (`id`, `user_id`, `off_date`, `lastmodify`, `status`) values (NULL,'" . $this->user_id . "', '" . $offdate . "', '".$this->lastmodify."','0')";
            $value=mysqli_query($this->conn,$result);
        }
                
        if ($value) {
            echo filter_var("Inserted");
            return true;
        }else{
	...
	function delete_monthoff() {
        ...
        for($i=1;$i<=$month_size;$i++) {
            $offdate = $this->off_year_month.'-'.$i;
            $result ="delete FROM `ld_off_days` WHERE `user_id` = '" . $this->user_id . "' and `off_date` = '" . $offdate . "'";
            $value=mysqli_query($this->conn,$result);
            
        }
                
        if ($value) {
            echo filter_var("Deleted");
            return true;
        } else {
            echo filter_var("Failed");
            return false;
	...
	for($i=1;$i<=$month_size;$i++) {
	$offdate = $this->off_year_month.'-'.$i;
	$query = "select `id` FROM  `ld_off_days` WHERE `user_id` = '" . $this->user_id . "' and `off_date` = '" . $offdate . "'";
	$result=mysqli_query($this->conn,$query);
	$value=mysqli_fetch_array($result);
