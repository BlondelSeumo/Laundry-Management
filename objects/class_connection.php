...
class laundry_db extends laundry_myvariable{
	...
	public function connect(){
        ...
        $this->password = base64_decode($this->passwords);
		...
		if(is_numeric(strpos($this->hostname,':'))){
			...
			$conn = new mysqli($this->hostname,$this->user,$this->password,$this->db_name,$this->mysqlport);
			...
			if (!mysqli_query($conn, "select curtime()")) {
			 ...
			 $conn = new mysqli($this->hostname,$this->user,$this->password,$this->db_name);
		}		
	...
	public function check_existing_tables_index($table_conn){
		$result = mysqli_query($table_conn,"SHOW TABLES LIKE 'ld_admin_info'");
		if($result->num_rows != 1){
			return 'table_not_exist';
		}else{
			$val=mysqli_query($table_conn,"select * from `ld_admin_info` LIMIT 1");
