...
if (isset($_POST['checkadmin'])) {
    $name = trim(strip_tags(mysqli_real_escape_string($conn,filter_var($_POST['name']))));
    $password = md5(filter_var($_POST['password']));
    $objlogin->remember = filter_var($_POST['remember']);
    $objlogin->cookie_passwords = filter_var($_POST['password']);
   ...
   /*  print_r($t);
    ...
    $objadmininfo->password = filter_var($_POST['retype_new_reset_pass']);
