...
if(isset($_POST['staff_payment_details'])){
	$all = $staff_commision->get_staff_detail(filter_var($_POST['staff_ids']));
	$i = 1;
	if(mysqli_num_rows($all)>0){
		while($a = mysqli_fetch_array($all)){
		?>
			<tr>
				<td><?php echo filter_var($i); ?><input type="hidden" class="staff_pymnt_id" value="<?php echo filter_var($a['id']); ?>" /><input type="hidden" class="staff_pymnt_orderid" value="<?php echo filter_var($_POST['order_id']); ?>" /></td>
				<td><?php echo filter_var($a['fullname']); ?></td>
				<td>
					<div class="input-group">
						<span class="input-group-addon"><?php echo filter_var($ld_currency_symbol); ?></span>
						<input type="text" class="form-control sp_staff_amount" value="0" id="sp_staff_amount<?php  echo filter_var($a['id']); ?>" data-id="<?php echo filter_var($a['id']); ?>" placeholder="00">
					</div>
				</td>
				<td>
					<div class="input-group">
						<span class="input-group-addon"><?php echo filter_var($ld_currency_symbol); ?></span>
						<input type="text" class="form-control sp_staff_advance_paid" value="0" id="sp_staff_advance_paid<?php  echo filter_var($a['id']); ?>" data-id="<?php echo filter_var($a['id']); ?>" placeholder="00">
					</div>
				</td>
				<td>
					<div class="input-group">
						<span><?php echo filter_var($ld_currency_symbol); ?><span class="sp_staff_net_total<?php  echo filter_var($a['id']); ?>">0</span></span>
						<input type="hidden" class="form-control sp_staff_net_total" value="0" id="sp_staff_net_total<?php  echo filter_var($a['id']); ?>" data-id="<?php echo filter_var($a['id']); ?>" placeholder="00">
					</div>
				</td>
			</tr>
		<?php 
...
if(isset($_POST['staff_payment_save'])){
	$data_staff_pymnt_id = explode(',',filter_var($_POST['staff_pymnt_id']));
	$data_sp_staff_amount = explode(',',filter_var($_POST['sp_staff_amount']));
	$data_sp_staff_advance_paid = explode(',',filter_var($_POST['sp_staff_advance_paid']));
	$data_sp_staff_net_total = explode(',',filter_var($_POST['sp_staff_net_total']));
	$order_id = filter_var($_POST['staff_pymnt_orderid']);
