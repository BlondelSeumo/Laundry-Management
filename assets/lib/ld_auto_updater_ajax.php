...
if (extension_loaded('zip')) {
	...
	$server_path = str_rot13("uggc://fxlzbbaynof.pbz/ynhaqel/");
 
	/* download zip */
	if(isset($_POST['action']) && $_POST['action'] == "auto_updater")
	{
		$getVersions = $settings->ext_get_contents('http://skymoonlabs.com/laundry/versioncheck.php?'.time());
		if ($getVersions != '')
		{
			/* If we managed to access that file, then lets break up those release versions into an array. */
			echo '<p>CURRENT VERSION: '.$settings->get_option('ld_version').'</p>';
			echo '<p>Reading Current Releases List</p>';
			$versionList = explode("n", $getVersions);    
			foreach ($versionList as $aV)
			{
				if ( $aV == $settings->get_option('ld_version')) 
				{
					echo '<p>New Update Found: v'.$aV.'</p>';
					$found = true;
					/* Download The File If We Do Not Have It */
					if ( !is_file(dirname(dirname(dirname(__FILE__))).'/ld-updates/ld-'.$aV.'.zip' )) 
					{
						echo '<p>Downloading New Update</p>';
						$newUpdate = $settings->url_get_contents($server_path.'ld-updates/ld-'.$aV.'.zip');
						if ( !is_dir( dirname(dirname(dirname(__FILE__))).'/ld-updates/' ) ) 
							mkdir ( dirname(dirname(dirname(__FILE__))).'/ld-updates/' );
						
						$dlHandler = fopen(dirname(dirname(dirname(__FILE__))).'/ld-updates/ld-'.$aV.'.zip', 'w');
					...
					{ 
						...
						$zipHandle = zip_open(dirname(dirname(dirname(__FILE__))).'/ld-updates/ld-'.$aV.'.zip');
			...
			if ($updated == true)
			{
				echo '<p class="success">&raquo; Laundry Updated to v'.$aV.'</p>';
			}
