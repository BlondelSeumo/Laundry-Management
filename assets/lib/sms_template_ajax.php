...
if(isset($_POST['save_sms_template'])){
    $sms_template->id = filter_var($_POST['id']);
    $sms_template->sms_message = base64_encode(filter_var($_POST['sms_messages']));
    $updated = $sms_template->update_sms_template();
}
else if(isset($_POST['save_sms_template_status'])){
    $sms_template->id = filter_var($_POST['id']);
    $sms_template->sms_template_status = filter_var($_POST['sms_template_status']);
